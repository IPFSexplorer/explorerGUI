<template>
    <v-card class="mx-auto my-3" max-width="650" outlined>
        <v-list-item three-line>
            <v-list-item-content>
                ...
                <v-list-item-subtitle
                    >Block
                    <router-link :to="{ name: 'BlockDetail', params: { blockHeight: transaction.blockHeight } }"
        ...
        <v-divider></v-divider>
        <v-row class="px-2">
            <v-col cols="6" class="border"><h3>Inputs</h3></v-col>
            <v-col cols="6"><h3>Outputs</h3></v-col>
        </v-row>
        <v-divider></v-divider>
        <v-row class="px-2">
            <v-col cols="6" class="border">
                <v-row v-for="(input, i) in transaction.vin" :key="i">
                    ...
                    <v-col cols="7" class="overflow">
                        <router-link v-if="input.isAddress" :to="'/'">{{ input.addresses[0] }}</router-link>
                        <template v-else>No address</template>
                    </v-col>
                    <v-col cols="4" v-if="input.value">{{ input.value / 100000000 }} {{ currency.unit }}</v-col>
                </v-row>
            </v-col>
            <v-col cols="6">
                <v-row v-for="(output, i) in transaction.vout" :key="i">
                    <v-col cols="7" class="overflow">
                        <router-link v-if="output.isAddress" :to="'/'">{{ output.addresses[0] }}</router-link>
                        <template v-else>No address</template>
                    </v-col>
                    <v-col cols="4" v-if="output.value">{{ output.value / 100000000 }} {{ currency.unit }}</v-col>
